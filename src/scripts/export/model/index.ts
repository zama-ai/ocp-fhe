...
class Model implements WorkbookModel {
    ...
    private issuedSecuritiesByStakeholderAndStockClassIds_ = new Map<string, Set<string>>();
    private issuedSecuritiesByStakeholderAndStockPlanIds_ = new Map<string, Set<string>>();
    private issuedSecuritiesByStakeholderAndWarrantStockIds_ = new Map<string, Set<string>>();

    private issuedSecuritiesByStakeholderAndNonPlanStockIds_ = new Map<string, Set<string>>();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    private adjustmentsByStockPlanId_ = new Map<string, Set<any>>();

    constructor(public readonly asOfDate: Date, public readonly generatedAtTimestamp: Date) {}
    ...
    public get stockClasses() {
        if (this.sortedStockClasses_.length !== this.stockClasses_.length) {
            this.sortedStockClasses_ = [...this.stockClasses_].sort(this.compareClassesForSort);
        }
    ...
    public get stockPlans() {
        if (this.sortedStockPlans_.length !== this.stockPlans_.length) {
            this.sortedStockPlans_ = [...this.stockPlans_].sort(this.comparePlansForSort);
        }

        return this.sortedStockPlans_;
    }

    public getStakeholderStockHoldings(stakeholder: StakeholderModel, stockClass: WorkbookStockClassModel) {
        const calculator = new OutstandingStockSharesCalculator();

        const issuanceSecurityIds = this.issuedSecuritiesByStakeholderAndStockClassIds_.get(`${stakeholder.id}/${stockClass.id}`) || new Set();
    ...
    public getStakeholderStockPlanHoldings(stakeholder: StakeholderModel, stockPlan: WorkbookStockPlanModel) {
        const calculator = new OutstandingStockPlanCalculator();

        // get stock class for this plan
        // get ratio if stock class is preferred
        const stockClass = this.stockClasses_.find((cls) => cls.id === stockPlan.stock_class_id);
        let ratio = 1;
        if (stockClass?.is_preferred) {
            ratio = this.getStockClassConversionRatio(stockClass);
        }

        const issuanceSecurityIds = this.issuedSecuritiesByStakeholderAndStockPlanIds_.get(`${stakeholder.id}/${stockPlan.id}`) || new Set();
        for (const id of issuanceSecurityIds) {
            for (const txn of this.transactionsBySecurityId_.get(id) || []) {
                calculator.apply(txn);
            }
    ...
    public getStakeholderWarrantHoldings(stakeholder: StakeholderModel, stockClass: WorkbookStockClassModel) {
        ...
        const issuanceSecurityIds = this.issuedSecuritiesByStakeholderAndWarrantStockIds_.get(`${stakeholder.id}/${stockClass.id}`) || new Set();
    ...
    public getStakeholderNonPlanHoldings(stakeholder: StakeholderModel, stockClass: WorkbookStockClassModel) {
        const calculator = new OutstandingStockPlanCalculator();
        // get ratio if stock class is preferred
        let ratio = 1;
        if (stockClass?.is_preferred) {
        ...
        const issuanceSecurityIds = this.issuedSecuritiesByStakeholderAndNonPlanStockIds_.get(`${stakeholder.id}/${stockClass.id}`) || new Set();
    ...
    private compareClassesForSort(classA: StockClassModel, classB: StockClassModel) {
        ...
        const now = new Date();
        const dateDiff: number = (classA.board_approval_date ?? now).valueOf() - (classB.board_approval_date ?? now).valueOf();
    ...
    private STOCK_CLASS(value: any) {
        ...
        const rounding_type = this.getStockClassRoundingType(value?.conversion_rights);
    ...
    private TX_STOCK(value: any) {
        if (value.object_type === "TX_STOCK_ISSUANCE") {
            ...
            const ids = this.issuedSecuritiesByStakeholderAndStockClassIds_.get(key) || new Set();
            ids.add(value.security_id);
            this.issuedSecuritiesByStakeholderAndStockClassIds_.set(key, ids);
        }
        const txns = this.transactionsBySecurityId_.get(value.security_id) || new Set();
        txns.add(value);
    ...
    private TX_WARRANT(value: any) {
        if (value.object_type === "TX_WARRANT_ISSUANCE") {
            ...
            if (stock_class_id !== undefined) {
                ...
                const ids = this.issuedSecuritiesByStakeholderAndWarrantStockIds_.get(key) || new Set();
                ids.add(value.security_id);
                this.issuedSecuritiesByStakeholderAndWarrantStockIds_.set(key, ids);
                this.warrantStockIds.add(stock_class_id);
                const source = value.quantity_source !== undefined ? value.quantity_source : "UNSPECIFIED";
    ...
    private TX_STOCK_PLAN_POOL_ADJUSTMENT(value: any) {
        const txns = this.adjustmentsByStockPlanId_.get(value.stock_plan_id) || new Set();
        txns.add(value);
    ...
    private TX_PLAN_SECURITY(value: any) {
        if (value.object_type === "TX_PLAN_SECURITY_ISSUANCE") {
            ...
            const ids = this.issuedSecuritiesByStakeholderAndStockPlanIds_.get(key) || new Set();
            ids.add(value.security_id);
    ...
    private TX_EQUITY_COMPENSATION(value: any) {
        if (value.object_type === "TX_EQUITY_COMPENSATION_ISSUANCE" && value.stock_class_id !== undefined) {
            const key = `${value.stakeholder_id}/${value.stock_class_id}`;
            const ids = this.issuedSecuritiesByStakeholderAndNonPlanStockIds_.get(key) || new Set();
            ids.add(value.security_id);
