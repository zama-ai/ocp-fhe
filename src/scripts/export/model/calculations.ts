...
function convertRatioToDecimalNumber(ratio: { numerator: string; denominator: string }) {
...
export class OutstandingStockPlanCalculator extends OutstandingEquityCalculatorBase {
    public apply(txn: Transaction): void {
        const operand = txn.quantity ?? txn.quantity_converted ?? "0";
        if (txn.object_type === "TX_PLAN_SECURITY_ISSUANCE" || txn.object_type === "TX_EQUITY_COMPENSATION_ISSUANCE") {
            this.value_ = this.value_.plus(operand);
            this.issuanceAmounts_.set(txn.security_id, operand);
        } else if (txn.object_type === "TX_PLAN_SECURITY_RETRACTION" || txn.object_type === "TX_EQUITY_COMPENSATION_RETRACTION") {
            this.pendingSecurityIds_.add(txn.security_id);
        } else {
            this.value_ = this.value_.minus(operand);
        }
...
export class WarrantSharesCalculator extends OutstandingEquityCalculatorBase {
    public apply(txn: Transaction): void {
        ...
        if (txn.object_type === "TX_WARRANT_ISSUANCE") {
            this.value_ = this.value_.plus(operand);
            this.issuanceAmounts_.set(txn.security_id, operand);
        } else if (txn.object_type === "TX_WARRANT_RETRACTION" || txn.object_type === "TX_WARRANT_EXERCISE") {
            this.pendingSecurityIds_.add(txn.security_id);
        } else {
            this.value_ = this.value_.minus(operand);
        }
    ...
    public getQuantity(txn: Transaction): string {
        ...
        if (txn.exercise_triggers !== undefined) {
            for (const trigger of txn.exercise_triggers) {
                if (trigger?.conversion_right?.conversion_mechanism?.converts_to_quantity !== undefined) {
                    return trigger.conversion_right.conversion_mechanism.converts_to_quantity;
                }
            }
        }
        return "0";
...
export class OptionsRemainingCalculator {
    ...
    public apply(initial_shares_reserved: string, total_holdings: number, adjustments?: Set<any>): void {
        if (adjustments && adjustments.size > 0) {
            const array_of_adjustments = Array.from(adjustments);
            array_of_adjustments.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
            ...
            this.value_ = new Big(initial_shares_reserved).minus(new Big(total_holdings));
        }
    }
}
...
export class ConversionRatioCalculator {
    ...
    public findRatio(preferredStockClassId: string): {
        ...
        const rootPreferredStockClass = this.stockClasses.find((cls) => cls.id === preferredStockClassId && cls.class_type === "PREFERRED");
        if (rootPreferredStockClass === undefined) {
            throw new Error("Preferred Stock Class not found.");
        }

        const queue: { stockClass: StockClass; currentPath: StockClass[] }[] = [{ stockClass: rootPreferredStockClass, currentPath: [] }];
        ...
        while (queue.length > 0) {
            ...
            if (dequeuedClass) {
                ...
                if (!visited.has(stockClass.id)) {
                    ...
                    if (stockClass?.class_type === "PREFERRED" && stockClass?.conversion_rights && stockClass?.conversion_rights.length > 0) {
                        for (const conversionRight of stockClass.conversion_rights) {
                            if (conversionRight.converts_to_stock_class_id && !visited.has(conversionRight.converts_to_stock_class_id)) {
                                const nextStockClass = conversionRight.converts_to_stock_class_id
                                    ? this.stockClasses.find((cls) => cls.id === conversionRight.converts_to_stock_class_id)
                                    : null;
                                if (nextStockClass) {
                                    queue.push({
    ...
    private getFinalRatio(path: StockClass[]): number {
        ...
        for (const stockClass of path) {
            info += ` ${stockClass.name} ${path.indexOf(stockClass) < path.length - 1 ? ">" : ""}`;
            if (stockClass.class_type === "COMMON") {
                const index = path.indexOf(stockClass);
                const preferredClass = path[index - 1];
                const conversionRight = preferredClass.conversion_rights?.find((cls) => cls.converts_to_stock_class_id === stockClass.id);
                const ratioObject = conversionRight?.conversion_mechanism?.ratio;
                const ratio = ratioObject !== undefined ? Calculations.convertRatioToDecimalNumber(ratioObject) : 1;
                ratios.push(ratio);
            }
        }

        const finalRatio: number = ratios.reduce((accumulator, currentValue) => accumulator * currentValue);
