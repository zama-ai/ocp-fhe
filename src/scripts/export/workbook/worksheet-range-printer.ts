...
abstract class WorksheetRangePrinter {
    ...
    public getExtents() {
        return {
            ...
            btmRightAddress: Extents.ExcelUtils.cursorToAddress(this.extents.btmRight),
     ...
     */
    public static create(printer: WorksheetLinePrinter, orientation: RangePrinterOrientation): WorksheetRangePrinter {
    ...
    public createNestedRange(
        ...
        if (this.extents.topLeft.row !== this.extents.btmRight.row || this.extents.topLeft.col !== this.extents.btmRight.col) {
        ...
        const range = WorksheetRangePrinter.createWithCursor(this.printer, resolvedOpts.orientation, this.cursor, this);
    ...
    public addCell(value: Date | string | number, style?: Partial<Style>): WorksheetRangePrinter {
    ...
    public addFormulaCell(formula: string, style?: Partial<Style>): WorksheetRangePrinter {
        this.printer.setFormulaCellAtCursor(this.cursor.row, this.cursor.col, `=${formula}`, { ...this.style, ...style });
        this.checkExtents();
        this.advanceCursor();
        return this;
    }

    public addRepeatedFormulaCell(formula: string, repetitions: number, style?: Partial<Style>) {
        this.printer.setFormulaCellAtCursor(this.cursor.row, this.cursor.col, `=${formula}`, { ...this.style, ...style });

        const referenceFormulaAddress = Extents.ExcelUtils.cursorToAddress(this.cursor);

        for (let idx = 1; idx < repetitions; idx++) {
            this.advanceCursor();
            this.printer.copyFormulaCell(referenceFormulaAddress, this.cursor.row, this.cursor.col, { ...this.style, ...style });
        }
    ...
    public addBlankCells(n: number, style?: Partial<Style>): WorksheetRangePrinter {
    ...
    public addSumFor(otherRange: WorksheetRangePrinter, style?: Partial<Style>) {
        const topLeftCell = Extents.ExcelUtils.cursorToAddress(otherRange.extents.topLeft);
        const bottomRightCell = Extents.ExcelUtils.cursorToAddress(otherRange.extents.btmRight);
