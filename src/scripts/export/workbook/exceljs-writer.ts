...
class ExcelJSLinePrinter {
    ...
    public setCellAtCursor(row: number, col: number, value: Date | string | number | null, style?: Partial<Style>) {
    ...
    public setFormulaCellAtCursor(row: number, col: number, formula: string, style?: Partial<Style>) {
    ...
    public copyFormulaCell(from: string, row: number, col: number, style?: Partial<Style>) {
        this.worksheet.getCell(row, col).value = {
            sharedFormula: from,
            date1904: false,
        };
