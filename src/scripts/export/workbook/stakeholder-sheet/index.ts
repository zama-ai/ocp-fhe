import { Model, StockClassModel, StockPlanModel, WorksheetLinePrinter } from "../interfaces";
...
class StakeholderSheet {
    ...
    private createCapitalizationByStakeholderTable() {
        ...
        new CapitalizationByStakeholderHeader(this.sheet).write(this.model.issuerName, stockColumns.length);

        const holdingsTable = this.sheet.createNestedRange({
            orientation: "left-to-right",
        });
        new Holdings.StakeholderColumn(holdingsTable).write(this.model.stakeholders);
        new Holdings.StakeholderGroupColumn(holdingsTable).write(this.model.stakeholders);
        ...
        for (let idx = 0; idx < this.stockClasses.length; ++idx) {
            ...
            const outstandingRange = new Holdings.StockClassOutstandingColumn(holdingsTable).write(stockClass, this.model);
            outstandingRanges.push(outstandingRange);
            const ratio = this.model.getStockClassConversionRatio ? this.model.getStockClassConversionRatio(stockClass) : 1.0;
            if (!stockClass.is_preferred || (stockClass.is_preferred && ratio === 1.0)) {
                fullyDilutedRanges.push(outstandingRange.getExtents());
            }

            if (stockClass.is_preferred && ratio !== 1.0) {
                const convertedRange = new Holdings.StockClassAsConvertedColumn(holdingsTable).write(stockClass, outstandingRange, this.model);
        ...
        for (const plan of this.stockPlans) {
            const stockPlanRange = new Holdings.StockPlanColumn(holdingsTable).write(plan, this.model);
            fullyDilutedRanges.push(stockPlanRange.getExtents());
        }

        const warrantStockClasses = [];
        for (const id of this.warrantStockIds) {
            const stockClass = this.stockClasses.find((stockClass) => stockClass.id === id);
            if (stockClass !== undefined) {
                warrantStockClasses.push(stockClass);
            }
        }
        for (const stockClass of warrantStockClasses) {
            const warrantRange = new Holdings.WarrantColumn(holdingsTable).write(stockClass, this.model);
        ...
        for (const stockClass of nonPlanStockClasses) {
            const nonPlanRange = new Holdings.NonPlanColumn(holdingsTable).write(stockClass, this.model);
    ...
    private createNotesTable() {
        ...
        for (const id of this.warrantStockIds) {
            const source = this.warrantsSources instanceof Map ? this.warrantsSources.get(id) : "UNSPECIFIED";
