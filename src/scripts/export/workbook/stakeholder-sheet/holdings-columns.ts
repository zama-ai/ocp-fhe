...
import { Model, StockClassModel, StockPlanModel, StakeholderModel } from "../interfaces";
...
export class StockClassOutstandingColumn {
    ...
    public write(stockClass: StockClassModel, model: Model): WorksheetRangePrinter {
        const myColumn = this.parent.createNestedRange({
            orientation: "top-to-bottom",
        });
        ...
        model.stakeholders.forEach((s) => {
            const holding = model.getStakeholderStockHoldings ? model.getStakeholderStockHoldings(s, stockClass) : 0;
        ...
        myColumn.setWidth(Math.max(14, (largestHolding * model.stakeholders.length).toString().length));
        return myData;
    }

    private outstandingStockClassHeadingFor(stockClass: StockClassModel, model: Model) {
        let suffix = "";
        if (stockClass.is_preferred) {
            const ratio = model.getStockClassConversionRatio ? model.getStockClassConversionRatio(stockClass) : 1;
            const ratioToFourPlaces = ratio.toFixed(4);
            suffix = `\n(outstanding) (${ratio !== Number(ratioToFourPlaces) ? "~" : ""}${ratioToFourPlaces})`;
        }
...
export class StockClassAsConvertedColumn {
    ...
    public write(stockClass: StockClassModel, outstandingRange: WorksheetRangePrinter, model: Model) {
        const myColumn = this.parent.createNestedRange({
            orientation: "top-to-bottom",
        });
        ...
        const conversion_value = `${outstandingRange.getExtents().topLeftAddress} * ${ratio}`;

        const formula = this.getRoundingFormula(stockClass, conversion_value);

        myData.addRepeatedFormulaCell(formula, outstandingRange.getExtents().height);
    ...
    private getRoundingFormula(stockClass: StockClassModel, conversion_value: string): string {
...
export class StockPlanColumn {
    ...
    public write(stockPlan: StockPlanModel, model: Model): WorksheetRangePrinter {
        ...
        let largestHolding = 0;
        model.stakeholders.forEach((s) => {
            const holding = model.getStakeholderStockPlanHoldings ? model.getStakeholderStockPlanHoldings(s, stockPlan) : 0;
            myData.addCell(holding);
            if (holding > largestHolding) {
                largestHolding = holding;
            }
        ...
        const options_remaining = model.getOptionsRemainingForIssuance ? model.getOptionsRemainingForIssuance(stockPlan) : 0;
...
export class WarrantColumn {
    ...
    public write(stockClass: StockClassModel, model: Model): WorksheetRangePrinter {
        ...
        if (stockClass.is_preferred) {
            targetClass = model.getConversionCommonStockClass ? model.getConversionCommonStockClass(stockClass) : stockClass;
        }
        ...
        let largestHolding = 0;
        model.stakeholders.forEach((s) => {
            const holding = model.getStakeholderWarrantHoldings ? model.getStakeholderWarrantHoldings(s, stockClass) : 0;
            myData.addCell(holding);
            if (holding > largestHolding) {
                largestHolding = holding;
            }
...
export class NonPlanColumn {
    ...
    public write(stockClass: StockClassModel, model: Model): WorksheetRangePrinter {
        ...
        let largestHolding = 0;
        model.stakeholders.forEach((s) => {
            const holding = model.getStakeholderNonPlanHoldings ? model.getStakeholderNonPlanHoldings(s, stockClass) : 0;
            myData.addCell(holding);
            if (holding > largestHolding) {
                largestHolding = holding;
            }
...
export class TotalOutstanding {
    ...
    public write(outstandingRanges: Array<WorksheetRangePrinter>) {
        ...
        const cellsToSum = outstandingRanges.map((o) => o.getExtents().topLeftAddress).join(",");
        const height = Math.max(...outstandingRanges.map((o) => o.getExtents().height));
...
export class TotalAsConverted {
    ...
    public write(sourceRanges: ExtentsCollection) {
        ...
        myColumn.setWidth(15);
        myColumn.break();
        this.writePercentagesCalculatedFrom(myData, myTotal.getExtents().btmRightAddress);
        return myData;
    }

    private writePercentagesCalculatedFrom(data: WorksheetRangePrinter, totalAddress: string) {
        const myColumn = this.parent.createNestedRange({
            orientation: "top-to-bottom",
        });
        ...
        const formula = `${data.getExtents().topLeftAddress} / ${totalAbsoluteAddress}`;
...
export class FullyDilutedShares {
    ...
    public write(fullyDilutedRanges: ExtentsCollection) {
        ...
        const cellsToSum = fullyDilutedRanges.map((o) => o.topLeftAddress).join(",");
        myData.addRepeatedFormulaCell(`SUM(${cellsToSum})`, fullyDilutedRanges.height);
...
export class WarrantsNotes {
    ...
    public write(source: string | undefined, notesObj: { value: number }) {
        ...
        const notes = "*".repeat(notesObj.value);
        const content = `${notes} {Notes re: warrant from vendor} ${this.getNotes(source)}`;
        myColumn.addCell(content, {
            ...Styles.text,
            alignment: { vertical: "middle", horizontal: "left", wrapText: true },
        });
