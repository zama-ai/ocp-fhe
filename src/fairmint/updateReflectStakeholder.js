...
export const updateReflectedStakeholder = async ({ issuerId, stakeholder }) => {
    ...
    const webHookUrl = `${API_URL}/ocp/updateReflectedStakeholder?portalId=${issuerId}`;
    ...
    try {
        ...
        console.log(`Successfully updated Stakeholder ${stakeholder._id} into Fairmint webhook`);
        ...
        if (error.response) {
            ...
            throw Error(`Error updating reflected Stakeholder into Fairmint: ${JSON.stringify(formattedError, null, 2)}`);
        } else {
            throw Error(`Error updating reflected Stakeholder into Fairmint: ${error.message}`);
        }
    }
};
