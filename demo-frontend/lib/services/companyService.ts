...
export class CompanyService {
    ...
    async createCompany(companyData: CompanyCreateData): Promise<Company> {
        try {
            ...
            // Index by founder
            const founderKey = generateKey(KEY_PREFIXES.COMPANY_BY_FOUNDER, companyData.founder);
    ...
    async getCompaniesByFounder(founderAddress: string): Promise<Company[]> {
        try {
            const founderKey = generateKey(KEY_PREFIXES.COMPANY_BY_FOUNDER, founderAddress);
    ...
    async getCompaniesByInvestor(investorAddress: string): Promise<Company[]> {
        try {
            const investorKey = generateKey(KEY_PREFIXES.COMPANY_BY_INVESTOR, investorAddress);
            ...
            return {
                ...
                rounds: (() => {
                    try {
                        ...
                        if (typeof companyData.rounds === 'string' && companyData.rounds !== '') {
                ...
                investors: (() => {
                    try {
                        ...
                        if (typeof companyData.investors === 'string' && companyData.investors !== '') {
     ...
     */
    async addRound(companyId: string, roundData: RoundCreateData): Promise<Company> {
        try {
     ...
     */
    async addInvestorToLastRound(companyId: string, investor: { address: string; name?: string }): Promise<Company> {
        try {
            const company = await this.getCompanyById(companyId);
            if (!company) throw new Error('Company not found');
            ...
            const investorKey = generateKey(KEY_PREFIXES.COMPANY_BY_INVESTOR, investor.address);
