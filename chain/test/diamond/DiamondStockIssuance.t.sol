...
contract DiamondStockIssuanceTest is DiamondTestBase {
    ...
    function testIssueStock() public {
        ...
        StockFacet(address(diamond)).issueStock(stockClassId, sharePrice, quantity, stakeholderId);
    }
    ...
    function testFailInvalidStakeholder() public {
        ...
        StockFacet(address(diamond)).issueStock(stockClassId, 10000000000, 1000, invalidStakeholderId);
    }
    ...
    function testFailInvalidStockClass() public {
        ...
        StockFacet(address(diamond)).issueStock(invalidStockClassId, 10000000000, 1000, stakeholderId);
    }
    ...
    function testFailInsufficientIssuerShares() public {
        ...
        StockFacet(address(diamond)).issueStock(stockClassId, 10000000000, 1000, stakeholderId);
    }

    function testFailInsufficientStockClassShares() public {
        (bytes16 stockClassId, bytes16 stakeholderId) = createStockClassAndStakeholder(100);

        StockFacet(address(diamond)).issueStock(stockClassId, 10000000000, 101, stakeholderId);
    }
}
