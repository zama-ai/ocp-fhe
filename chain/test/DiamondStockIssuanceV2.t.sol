...
contract DiamondStockIssuanceTest is Test {
    ...
    function testIssueStock() public {
        ...
        // Verify the position details on the NFT contract
        ActivePositionNFT.Position memory position = activePositionNFT.positions(tokenId);
        assertEq(position.quantity, 1000);
        assertEq(position.sharePrice, 10000000000);
    }
}
